...
import { fetchDepartmentsList, deleteDepartment } from "../../../redux/settings/departments/departmentsSlice";
...
const DepartmentsList = () => {
    ...
    const { departmentsList, departmentsMeta, loadingList } = useSelector(state => state.departments);
    ...
    const handleDeleteModalConfirm = () => {
        ...
        dispatch(deleteDepartment({id: itemForDelete.id, perPage, key, sortDirection, currentPage}));
    ...
    useEffect(() => {
        const {key, sortDirection} = activeSortData;
        dispatch(fetchDepartmentsList({perPage, key, sortDirection, currentPage}));
    ...
    return (
        <>
                ...
                <CCol>
                    <CCard>
                        <CCardHeader className="d-flex justify-content-between align-items-center bg-white">
                            {t('label.departments')}
                            {canWriteDepartment && (
                                <AddNewButton to="/settings/departments/add" label={t('label.add_new')} />
                            )}
                        </CCardHeader>
                        <CCardBody className='mb-100'>
                            <Table theadData={theadData} tbodyData={departmentsList} rowsPerPage={perPage} loadingTableData={loadingList} customClass={'table-striped'} onChangeColumnSort={handleColumnSort}></Table>
                            {!loadingList ? <Pagination meta={departmentsMeta} onHandlePerPage={handlePerPageChange} onHadlePage={handlePageChange}></Pagination> : null}
                        </CCardBody>
                    </CCard>
                </CCol>
            </CRow>
            ...
            <ConfirmModal
                ...
                modalBodyText={t('text.confirm_department_delete_body_text')}
