...
import { getSingleTeam } from "../../../services/http-services/settings/teams";
...
import { refreshErrors, editTeam } from '../../../redux/settings/teams/teamsSlice';

const EditTeam = () => {
    const { id } = useParams();
    const editTeamForm = useRef();
    ...
    const handleEditTeamSave = () => editTeamForm.current.submitForm();
    const handleEditTeamFormSubmit = (values) => {
        const reqBody = {...values};
        dispatch(editTeam({id, reqBody}));
    }
    ...
    useEffect(() => {
        if (errors && Object.keys(errors).length > 0) {
            editTeamForm.current.setSubmitting(false);
            editTeamForm.current.setErrors(errors);
            ...
            <CCol>
                <CCard>
                    ...
                    {!loadingData && <CCardBody className='mb-100'>
                        <Formik
                            innerRef={editTeamForm}
                            validationSchema={TEAMS_SETTINGS_FORM}
                            onSubmit={handleEditTeamFormSubmit}
                    ...
                    {(!loadingData && canWriteTeam) && (
                        <CCardFooter className="bg-white">
                            <CButton type="button" color="primary" className="is-btn me-3" onClick={handleEditTeamSave} disabled={isSubmitting}>{isSubmitting ? <Spinner smallSize={true}/> :  '' }{t('label.save')}</CButton>
