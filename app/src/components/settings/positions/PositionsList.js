...
import { fetchPositionsList, deletePosition } from "../../../redux/settings/positions/positionsSlice";
...
const PositionsList = () => {
    ...
    const { positionsList, positionsMeta, loadingList } = useSelector(state => state.positions);
    ...
    const handleDeleteModalConfirm = () => {
        ...
        dispatch(deletePosition({id: itemForDelete.id, perPage, key, sortDirection, currentPage}));
    ...
    useEffect(() => {
        const {key, sortDirection} = activeSortData;
        dispatch(fetchPositionsList({perPage, key, sortDirection, currentPage}));
                            ...
                            render: (item) => {
                                return (
                                    <span className="d-inline-flex">
                                        <Link 
                                            to={`/settings/positions/edit/${item.id}`} 
    ...
    return (
        <>
                ...
                <CCol>
                    <CCard>
                        <CCardHeader className="d-flex justify-content-between align-items-center bg-white">
                            {t('label.positions_management')}
                            {canWritePosition && (
                                <AddNewButton to="/settings/positions/add" label={t('label.add_new')} />
                            )}
                        </CCardHeader>
                        <CCardBody className='mb-100'>
                            <Table theadData={theadData} tbodyData={positionsList} rowsPerPage={perPage} loadingTableData={loadingList} customClass={'table-striped'} onChangeColumnSort={handleColumnSort}></Table>
                            {!loadingList ? <Pagination meta={positionsMeta} onHandlePerPage={handlePerPageChange} onHadlePage={handlePageChange}></Pagination> : null}
                        </CCardBody>
                    </CCard>
                </CCol>
            </CRow>
            ...
            <ConfirmModal
                ...
                modalBodyText={t('text.confirm_position_delete_body_text')}
